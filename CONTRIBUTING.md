# Contributing to ATick for Node.js
...
- **Report a bug** — open an issue with a minimal reproduction (input PDF if you
  can share one, the exact `atick.*` call and its options JSON, the full error
  message, your OS and `node --version`).
...
4. Environment: `atick.version()`, `node --version`, operating system and CPU arch.
...
3. Make sure the examples run (`node examples/sign_pfx.js`) and the docs build cleanly.
